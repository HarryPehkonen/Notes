...
async function resetState() {
  ...
  g.NotesApp = {
    updateNote: async () => ({
      data: { id: 1, title: "Saved", content: "Saved content", updated_at: new Date().toISOString() },
    }),
  };
}
...
Deno.test({ name: "SyncManager: should initialize successfully", ...opts, fn: async () => {
  await resetState();
...
Deno.test({ name: "SyncManager: should set online status", ...opts, fn: async () => {
  await resetState();
  await syncManager.init();
...
Deno.test({ name: "SyncManager: should save draft to IndexedDB", ...opts, fn: async () => {
  await resetState();
  await syncManager.init();
  ...
  const savePromise = syncManager.saveNote("123", { title: "Test", content: "Content", tags: [] });
...
Deno.test({ name: "SyncManager: should call updateNote", ...opts, fn: async () => {
  await resetState();
  await syncManager.init();
...
Deno.test({ name: "SyncManager: should clear draft after sync", ...opts, fn: async () => {
  await resetState();
  await syncManager.init();
...
Deno.test({ name: "SyncManager: should queue when offline", ...opts, fn: async () => {
  await resetState();
  await syncManager.init();
...
Deno.test({ name: "SyncManager: waitForSync resolves immediately when idle", ...opts, fn: async () => {
  await resetState();
  await syncManager.init();
...
Deno.test({ name: "SyncManager: waitForSync waits for active syncs", ...opts, fn: async () => {
  await resetState();
  await syncManager.init();
...
Deno.test({ name: "SyncManager: should report syncing state", ...opts, fn: async () => {
  await resetState();
  await syncManager.init();
...
Deno.test({ name: "SyncManager: should track pending count", ...opts, fn: async () => {
  await resetState();
  await syncManager.init();
  syncManager.isOnline = false;
...
Deno.test({ name: "SyncManager: should check hasUnsavedChanges", ...opts, fn: async () => {
  await resetState();
  await syncManager.init();
  syncManager.isOnline = false;
...
Deno.test({ name: "SyncManager: should emit sync-draft-saved event", ...opts, fn: async () => {
  await resetState();
  await syncManager.init();
...
Deno.test({ name: "SyncManager: should emit sync-completed event", ...opts, fn: async () => {
  await resetState();
  await syncManager.init();
...
Deno.test({ name: "SyncManager: should emit sync-pending when offline", ...opts, fn: async () => {
  await resetState();
  await syncManager.init();
  syncManager.isOnline = false;
