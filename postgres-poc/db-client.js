...
export class DatabaseClient {
    ...
    async createNote({ userId, title, content, tags = [] }) {
        return await this.transaction(async (tx) => {
            ...
            if (tags.length > 0) {
                await this.addTagsToNote(tx, note.id, userId, tags);
            }
    ...
    async updateNote(noteId, updates) {
        ...
        return await this.transaction(async (tx) => {
            ...
            if (tags !== undefined) {
                ...
                // Add new tags
                if (tags.length > 0) {
                    await this.addTagsToNote(tx, noteId, note.user_id, tags);
                }
            }
    ...
    parsePostgreSQLStatements(sql) {
        ...
        for (const stmt of cleanedStatements) {
            ...
            } else if (upperStmt.includes('CREATE OR REPLACE FUNCTION') || upperStmt.includes('CREATE FUNCTION')) {
                functions.push(stmt);
