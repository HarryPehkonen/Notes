...
async function runTests() {
  ...
  try {
    ...
    await test("Update Note and Create Version", async () => {
      ...
      console.log(
        `   Content length: ${originalNote.content.length} → ${updatedNote.content.length}`,
      );
    ...
    if (passedTests === testCount) {
      console.log(
        `${colors.green}${colors.bold}  All tests passed! PostgreSQL integration is working perfectly.${colors.reset}`,
      );
